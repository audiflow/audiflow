...
class SettingsIntentResolver {
  ...
  /// Attempts to resolve a settings change from raw transcription text
  /// by matching against registry synonyms. Used as fallback when the AI
  /// identifies `changeSettings` intent but provides no structured payload.
  SettingsResolution? resolveFromTranscription(
    String transcription, {
    required Map<String, String> currentValues,
  }) {
    final text = transcription.toLowerCase();

    // Find all settings whose synonyms appear in the transcription
    final matches = <(SettingMetadata, int)>[];
    for (final meta in _registry.allSettings) {
      ...
      if (0 < bestLength) {
        matches.add((meta, bestLength));
      }
    }

    if (matches.isEmpty) return null;

    // Sort by longest synonym match (most specific wins)
    matches.sort((a, b) => b.$2.compareTo(a.$2));

    // Detect direction keywords
    final direction = _detectDirection(text);

    if (matches.length == 1 || matches[0].$2 != matches[1].$2) {
      // Single best match
      final meta = matches.first.$1;
      final oldValue = currentValues[meta.key] ?? '';

      if (direction != null && meta.constraints is RangeConstraints) {
        // Relative change
        return _resolveRelative(
          key: meta.key,
          direction: direction,
      ...
      // Can't determine value — ask for confirmation with current value
      return SettingsResolution.confirm(
        key: meta.key,
        oldValue: oldValue,
        newValue: oldValue,
    ...
    // Multiple equally-good matches — disambiguate
    final candidates = matches.take(3).map((m) {
      final meta = m.$1;
      final oldValue = currentValues[meta.key] ?? '';
      return SettingsResolutionCandidate(
        key: meta.key,
  ...
  /// Detects increase/decrease intent from Japanese/English keywords.
  ChangeDirection? _detectDirection(String text) {
    ...
    for (final kw in increaseKeywords) {
      if (text.contains(kw)) return ChangeDirection.increase;
    }
    for (final kw in decreaseKeywords) {
      if (text.contains(kw)) return ChangeDirection.decrease;
    }
    return null;
  }
