  ...
  group('SeasonEpisodeExtractor', () {
    ...
    group('fromJson/toJson', () {
      test('creates from JSON config', () {
        ...
        final extractor = SeasonEpisodeExtractor.fromJson(json);
      ...
      test('converts to JSON', () {
        const extractor = SeasonEpisodeExtractor(
          source: 'title',
          pattern: r'【(\d+)-(\d+)】',
          seasonGroup: 1,
          episodeGroup: 2,
    ...
    group('extract', () {
      ...
      test('extracts from description when source is description', () {
        const extractor = SeasonEpisodeExtractor(
          source: 'description',
      ...
      test('handles custom capture groups', () {
        const extractor = SeasonEpisodeExtractor(
          source: 'title',
          pattern: r'E(\d+)S(\d+)', // Episode first, then season
          seasonGroup: 2,
          episodeGroup: 1,
        );
    ...
    group('COTEN RADIO specific tests', () {
      const cotenExtractor = SeasonEpisodeExtractor(
        source: 'title',
        pattern: r'【(\d+)-(\d+)】',
        seasonGroup: 1,
        episodeGroup: 2,
      ...
      test('extracts from regular COTEN RADIO episode', () {
        final episode = makeEpisode(
          title: '【62-15】何が変わった？【COTEN RADIO リンカン編15】',
          seasonNumber: 62,
          episodeNumber: 999, // RSS might have different value
        );
      ...
      test('extracts from COTEN RADIO short episode', () {
        final episode = makeEpisode(
          title: '【1-8】ニコラ・テスラと直流/交流対決の謎【COTEN RADIOショート ニコラ・テスラ編8】',
          seasonNumber: 1,
        );
        final result = cotenExtractor.extract(episode);
      ...
      test('extracts from 番外編 with full-width hash', () {
        final episode = makeEpisode(
      ...
      test('extracts from 番外編 with half-width hash', () {
        final episode = makeEpisode(
          title: '【番外編#100】歴史の話',
          seasonNumber: 0,
          episodeNumber: 100,
