...
- **Config updated upstream**: When the ecosystem republishes a podcast's preset with a new data version, the app detects the version bump on the next refresh, evicts the stale cached copy, and re-fetches. The listener sees the updated grouping on their next visit without any manual cache clearing.
