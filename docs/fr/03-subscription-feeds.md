...
- **Post-import feed sync**: After an import adds subscriptions, their feeds are fetched immediately and in parallel, bypassing the usual refresh timing window, so the metadata backfill below runs without waiting. The fetch is not awaited: the import summary appears at once and the library list, which is database-backed, fills in as each feed resolves.
