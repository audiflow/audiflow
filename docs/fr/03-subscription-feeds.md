...
- **Importing from the migration guide**: The "Migrate from another app" guide explains how to get an OPML file out of another podcast app, so its readers are the people most likely to be holding one. It therefore offers the same import action directly, at the top and again at the end of the page, and names where the import lives in settings for anyone who returns later. Both entry points lead to the identical picker, preview, and summary.
...
- **Artwork and details on imported podcasts**: An OPML file carries only a title and a feed URL, so a freshly imported podcast starts with no artwork, author, or description. The import fetches each newly added feed straight away, so those details fill themselves in within moments of the summary dialog rather than waiting for the next scheduled refresh; rows update one by one as each feed answers. Taken from the feed itself, they make the podcast look the same as one added through search — artwork in the library list, the player, and the queue. If a show later renames itself, the new author and description replace the old ones on the next refresh; artwork is only ever filled in when missing, never swapped out.
...
- **Post-import feed sync**: After an import adds subscriptions, their feeds are fetched immediately, bypassing the usual refresh timing window, so the metadata backfill below runs without waiting. Fetches run a few at a time rather than all at once, since each one parses a feed in its own isolate and a large import would otherwise start hundreds together. The fetch is not awaited: the import summary appears at once and the library list, which is database-backed, fills in as each feed resolves.
...
- **Best-effort import orchestration**: For each selected entry, skips it if already subscribed (matched by feed URL), otherwise subscribes it with a deterministic placeholder iTunes ID derived from the feed URL, and collects failures without aborting. The outcome is returned as a summary of succeeded, already-subscribed, and failed entries. The placeholder ID keeps the subscription record valid without a catalog lookup; the podcast's own feed supplies the missing details during the post-import sync, or on a later refresh if that sync fails.
- **Channel metadata backfill**: On every feed refresh, and on a podcast detail visit, the artwork, author, and description carried by the RSS channel are written back to the subscription, so OPML-imported podcasts gain the details their import could not supply. Author and description follow the feed — a non-empty channel value replaces the stored one — while artwork is only ever filled in when missing, because channel images are far larger than the search artwork already stored and are decoded at their intrinsic size. Blank channel values are ignored rather than written, so a sparse feed never erases details obtained from search, and no write happens when nothing changed. A subscription that has never had its channel read requests its feed unconditionally while it still lacks artwork, since a "304 Not Modified" response would skip the parse that supplies it; once the channel has been read the subscription returns to conditional requests, whether or not the feed turned out to carry an image.
...
- **Source docs**: `docs/plans/2026-02-15-opml-feature-design.md`, `docs/plans/2026-02-15-opml-implementation-plan.md`, `packages/audiflow_podcast/CLAUDE.md`, `packages/audiflow_app/lib/features/subscription/presentation/controllers/subscription_controller.dart`, `packages/audiflow_domain/lib/src/features/subscription/repositories/subscription_repository.dart`, `packages/audiflow_domain/lib/src/features/subscription/services/opml_parser_service.dart`, `packages/audiflow_domain/lib/src/features/subscription/services/opml_import_service.dart`, `packages/audiflow_app/lib/features/settings/presentation/controllers/opml_export_controller.dart`, `packages/audiflow_app/lib/features/settings/presentation/controllers/opml_import_controller.dart`, `packages/audiflow_app/lib/features/settings/presentation/controllers/opml_file_receiver_controller.dart`, `packages/audiflow_app/lib/features/settings/presentation/screens/opml_import_preview_screen.dart`, `packages/audiflow_domain/lib/src/features/feed/services/feed_sync_service.dart`
