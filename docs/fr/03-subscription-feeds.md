...
- **Artwork and details on imported podcasts**: An OPML file carries only a title and a feed URL, so a freshly imported podcast starts with no artwork, author, or description. Once its feed is refreshed, those details are taken from the feed itself and the podcast looks the same as one added through search — artwork in the library list, the player, and the queue. If a show later renames itself, the new author and description replace the old ones on the next refresh; artwork is only ever filled in when missing, never swapped out.
