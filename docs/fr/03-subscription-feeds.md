...
- **Channel metadata backfill**: On every feed refresh, the artwork, author, and description carried by the RSS channel are written back to the subscription. The feed is the source of truth: a non-empty channel value replaces the stored one, so OPML-imported podcasts gain the details their import could not supply and search-originated ones stay current when a show rebrands. Blank channel values are ignored rather than written, so a sparse feed never erases details obtained from search, and no write happens when nothing changed.
