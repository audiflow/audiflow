...
- Recovery / fallback: Every preference falls back to a defined default value when nothing has been stored yet. "Reset all data" stops playback, deletes downloaded audio files, clears every Isar collection in one transaction, and clears all SharedPreferences keys, so the app behaves like a fresh install afterwards: the privacy consent screen, analytics opt-in default, and review-prompt state all start over. File deletion runs first so a failure there leaves the database and preferences untouched.
