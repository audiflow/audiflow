...
- Edge / failure case: A forgotten PIN cannot be recovered in-app — the user is told that resetting the PIN requires "Reset all data" from Storage & Data. That reset is itself PIN-gated while Restricted Mode is on (otherwise a child could bypass the lock by wiping the PIN hash). The honest forgotten-PIN recovery path is OS-level: uninstall and reinstall the app. PIN entry rate-limits after 5 consecutive failures with exponential backoff up to 5 minutes. A universal link that resolves to a podcast or episode the user is already subscribed to opens normally, even while Restricted Mode is on. Background feed refresh continues to run for the subscribed set without prompting for a PIN.
