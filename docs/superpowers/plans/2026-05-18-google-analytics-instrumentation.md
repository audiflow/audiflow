...
**Architecture:** Abstract `AnalyticsService` interface lives in `audiflow_domain`. Firebase-backed implementation lives in `audiflow_app`. Identity is an anonymous install UUID persisted in SharedPreferences and applied to both Sentry (`user.id` + `install_id` tag) and GA (`setUserId`). Cross-user-stable identifiers for podcasts/episodes are the raw iTunes ID (or feedUrl for OPML imports) and raw RSS guid, joined with `podcast_title` / `episode_title` event-scoped dimensions so reports in the GA console read like "<channel> / <episode>" without external joins. feedUrl/guid/titles are all public RSS data; no PII is in scope. `station_id` stays hashed (`sha256(installId + ':' + stationName)`) because per-user station names are not RSS-public. All string params are truncated to GA4's 100-char limit at the event boundary. A settings-screen toggle controls `setAnalyticsCollectionEnabled`. Flavor flag `enableAnalytics` gates Firebase init.
